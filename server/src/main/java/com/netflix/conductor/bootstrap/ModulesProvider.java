...
public class ModulesProvider implements Provider<List<AbstractModule>> {
	...
	public List<AbstractModule> get() {
		AbstractModule resolvedModule = (AbstractModule) Modules.override(selectModulesToLoad())
				.with(configuration.getAdditionalModules());
	...
	private List<AbstractModule> selectModulesToLoad() {
		...
		try {
			...
			final String message = "Invalid db name: " + configuration.getDBString() + ", supported values are: "
					+ Arrays.toString(Configuration.DB.values());
		...
		try {
			...
			logger.info("External payload storage is not configured, provided: {}, supported values are: {}",
					externalPayloadStorageString, Arrays.toString(ExternalPayloadStorageType.values()), e);
		}
