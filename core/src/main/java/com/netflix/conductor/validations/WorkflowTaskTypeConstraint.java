...
/**
 * This constraint class validates following things. 1. Correct parameters are
 * set depending on task type.
 */
@Documented
	...
	class WorkflowTaskValidator implements ConstraintValidator<WorkflowTaskTypeConstraint, WorkflowTask> {
		...
		public boolean isValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			case TaskType.TASK_TYPE_AMQP_PUBLISH:
				...
				break;
			case TaskType.TASK_TYPE_AMQP_WAIT:
				valid = isAMQPPublishTaskValid(workflowTask, context);
				break;
			}
		...
		private boolean isEventTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			if (workflowTask.getSink() == null) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "sink", TaskType.TASK_TYPE_EVENT,
		...
		private boolean isDecisionTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			if (workflowTask.getCaseValueParam() == null && workflowTask.getCaseExpression() == null) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "caseValueParam or caseExpression",
						TaskType.DECISION, workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
			if (workflowTask.getDecisionCases() == null) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "decisionCases", TaskType.DECISION,
						workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
				...
				String message = String.format(
						"decisionCases should have atleast one task for taskType: %s taskName: %s", TaskType.DECISION,
						workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
		...
		private boolean isDynamicForkJoinValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			// of dynamicForkTasksParam and dynamicForkTasksInputParamName.
			// Both are not allowed.
			if (workflowTask.getDynamicForkJoinTasksParam() != null && (workflowTask.getDynamicForkTasksParam() != null
			...
			if (workflowTask.getDynamicForkJoinTasksParam() != null) {
				...
				if (workflowTask.getDynamicForkTasksParam() == null) {
					String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "dynamicForkTasksParam",
							TaskType.FORK_JOIN_DYNAMIC, workflowTask.getName());
					context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
					valid = false;
				}
		...
		private boolean isHttpTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			// inputTemplate Taskdef should be set
			...
			if (!(isInputParameterSet || isInputTemplateSet)) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "inputParameters.http_request",
						TaskType.HTTP, workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
		...
		private boolean isForkJoinTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			if (workflowTask.getForkTasks() != null && (workflowTask.getForkTasks().size() == 0)) {
				String message = String.format("forkTasks should have atleast one task for taskType: %s taskName: %s",
		...
		private boolean isTerminateTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			if (workflowTask.isOptional()) {
				String message = String.format("terminate task cannot be optional, taskName: %s",
						workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
		...
		private boolean isKafkaPublishTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			if (!(isInputParameterSet || isInputTemplateSet)) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "inputParameters.kafka_request",
						TaskType.KAFKA_PUBLISH, workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
		...
		private boolean isAMQPPublishTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			...
			// inputTemplate Taskdef should be set
			if (workflowTask.getInputParameters() != null
					&& workflowTask.getInputParameters().containsKey("amqp_request")) {
				isInputParameterSet = true;
			}
			...
			if (taskDef != null && taskDef.getInputTemplate() != null
					&& taskDef.getInputTemplate().containsKey("amqp_request")) {
				isInputTemplateSet = true;
			}

			if (!(isInputParameterSet || isInputTemplateSet)) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "inputParameters.amqp_request",
						TaskType.AMQP_PUBLISH, workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
		...
		private boolean isAMQPWaitTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
			boolean valid = true;
			boolean isInputParameterSet = false;
			boolean isInputTemplateSet = false;
			...
			if (!(isInputParameterSet || isInputTemplateSet)) {
				String message = String.format(PARAM_REQUIRED_STRING_FORMAT, "inputParameters.amqp_request",
						TaskType.AMQP_WAIT, workflowTask.getName());
				context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
				valid = false;
			}
